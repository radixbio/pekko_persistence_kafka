...
object defns {
  ...
  case class SensorDetails(name: String, uuid: String, model: String, location: String) {
    def toStruct: Struct =
      new Struct(SensorDetails.kcSchema)
        .put("name", name)
        .put("uuid", uuid)
        .put("model", model)
  ...
  case class ElementA(
    ...
    override protected def finalizeStruct(base: Struct): Struct =
      base
  ...
  def convertSchema(raw: ASchema): KCSchema = {
    ...
    raw.getType match {
      ...
      case Type.UNION =>
        ...
        if (types.size() == 1) {
          ...
          nonNullType match {
            ...
            case other =>
              throw new IllegalArgumentException(s"Kafka connect does not support optional ${other.toString} types")
          }
        } else {
      ...
      case Type.RECORD =>
        ...
        raw.getFields.asScala
          .foldLeft(builder) { (build, field) =>
