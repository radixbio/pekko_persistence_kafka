...
object protocol {
  ...
  case class IsomorphismFind[T: Encoder: Decoder: SchemaFor: ClassTag](largerGraph: Graph[T],
                                                                       subGraph: Graph[T],
  ...
  object IsomorphismFind {
    implicit def avro[T: Encoder: Decoder: SchemaFor: ClassTag](implicit A: ExtendedActorSystem): AvroSerializer[IsomorphismFind[T]] = new AvroSerializer[IsomorphismFind[T]]
  }
  ...
  object IsomorphismResponse {
    implicit def avro[T: Encoder: Decoder: SchemaFor: ClassTag]: AvroSerializer[IsomorphismResponse[T]] = new AvroSerializer[IsomorphismResponse[T]]
  }
}
